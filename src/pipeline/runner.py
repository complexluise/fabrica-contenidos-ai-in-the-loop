...
from .assemble import concat_clips, trim_to
from .audio import (
    DEFAULT_VOICE_MODEL,
    effective_caption,
    mux_voiceover,
...
async def _render_shot(*, project, spec, cfg, run, keyframer, gate, tts,
                       scene, shot, shot_id, idx, refs, ref_sig,
                       rule, subset, strategy, provider_sig, keyframe_overrides):
    """Renderiza UN plano (D-028): keyframe -> video -> recorte -> caption/vo.
    ...
    Devuelve (clip_path, SceneRecord, manifest_entry, vo_applied).
    """
    ...
    return clip_path, record, manifest_entry, vo_applied
...
async def run_project(project: Project, spec: ProjectSpec, cfg: Config,
    ...
    clips = []
    vo_applied = False  # ¿se mezcló alguna VO de verdad? (para decidir el ducking)
    ...
    for scene in spec.scenes:
        ...
        for idx, shot in enumerate(shots):
            ...
            try:  # robustez: un plano que falla no aborta el run
                clip_path, record, mentry, vo = await _render_shot(
                    project=project, spec=spec, cfg=cfg, run=run, keyframer=keyframer,
                    gate=gate, tts=tts, scene=scene, shot=shot, shot_id=shot_id, idx=idx,
    ...
    # Con voz en off, la música baja para que la narración quede por encima (ducking).
    music_volume = 0.25 if vo_applied else 1.0
