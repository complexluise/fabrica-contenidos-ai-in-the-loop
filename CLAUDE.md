...
- **Interactive studio (`studio.py` + `runner.py` + `contact_sheet.py`).** `studio.py` implements cast/keyframes/pick/render against project state. `runner.py::run_project` runs the **pixel-real shot ribbon** (D-059): scenes flatten into an ordered sequence of shots; each chained shot starts from the **real last frame** of the previous clip (`extract_last_frame`) and lands on its chosen keyframe as `end_image` (the keyframe is the shot's **destination**, not frame-0). `transition: cut/smash_cut/wipe` breaks the chain; `match_cut/dissolve/None` continues it. Video is **sequential** by construction (revises D-039). Per-shot try/except so **one failed shot does not abort the run** (it cuts the chain; failures land in `run_report.json` via `telemetry.record_failure`); it raises only if *every* shot fails. Captions (`post.py::burn_lower_third`) and background music (`assemble.py::concat_clips(..., music=...)`) are best-effort.
